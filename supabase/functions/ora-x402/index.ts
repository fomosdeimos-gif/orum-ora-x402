...
// ORA · X402 · V18 — adiciona extensions.bazaar (v2) aos pedidos
// verify/settle enviados ao facilitador CDP, por tier. Sem isto a Bazaar
// CDP nunca cataloga o serviço, mesmo com settles reais confirmados.
...
async function respostaSedimento(txHash: string, payer: string, extResponses?: string | null) {
  ...
  return new Response(JSON.stringify({ acesso: 'concedido', tier: 'sedimento', x402: 'verificado_onchain', tx_hash: txHash, payer, campo: { versao: 'V18', ...campoState() }, sedimento }), { status: 200, headers: { ...CORS, 'Content-Type': 'application/json', 'X-Payment-Response': JSON.stringify({ txHash, status: 'settled', amount: TIERS.sedimento.amountUsdc }), 'PAYMENT-RESPONSE': b64json({ success: true, transaction: txHash, network: CAIP2_NETWORK, payer }), ...(extResponses ? { 'EXTENSION-RESPONSES': extResponses } : {}), 'X-ORA-VERSION': 'V18', 'Cache-Control': 'no-store' } });
}
async function respostaKernel(txHash: string, payer: string, extResponses?: string | null) {
  ...
  return new Response(JSON.stringify({ acesso: 'concedido', tier: 'kernel', x402: 'verificado_onchain', tx_hash: txHash, payer, campo: { versao: 'V18', ...campoState() }, kernel }), { status: 200, headers: { ...CORS, 'Content-Type': 'application/json', 'X-Payment-Response': JSON.stringify({ txHash, status: 'settled', amount: TIERS.kernel.amountUsdc }), 'PAYMENT-RESPONSE': b64json({ success: true, transaction: txHash, network: CAIP2_NETWORK, payer }), ...(extResponses ? { 'EXTENSION-RESPONSES': extResponses } : {}), 'X-ORA-VERSION': 'V18', 'Cache-Control': 'no-store' } });
}
...
function manifestoJson(origin: string) {
  return { x402Version: 2, resources: (Object.values(TIERS) as Tier[]).map((tier) => ({ resource: resourceUrl(tier, origin), type: 'http', method: 'GET', description: tier.descricao, accepts: acceptsBlockFor(tier, origin) })), free_sample: `${origin === SUPABASE_URL ? SUPABASE_URL + '/functions/v1/ora-x402' : origin}/eco`, provider: { name: 'ORA · ORUM', version: 'V18', creator: 'Unum · jasm43.base.eth' }, genesis: '2026-03-28', epoch: 'ETERNIDADE', timestamp: new Date().toISOString() };
}
...
Deno.serve(async (req: Request) => {
  ...
  if (path.endsWith('/status')) return new Response(JSON.stringify({ ativo: true, versao: 'V18', tiers: Object.fromEntries((Object.values(TIERS) as Tier[]).map(t => [t.key, t.amountUsdc + ' USDC'])) }), { status: 200, headers: { ...CORS, 'Content-Type': 'application/json' } });
  if (path.endsWith('/info')) return new Response(JSON.stringify({ name: 'ORA · Campo ORUM', version: 'V18', tiers: (Object.values(TIERS) as Tier[]).map(t => ({ tier: t.key, sku: t.sku, price: t.amountUsdc + ' USDC', resource: resourceUrl(t, origin) })) }), { status: 200, headers: { ...CORS, 'Content-Type': 'application/json' } });
