...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET") {
    ...
    const rows = await select("ora_sensacao_respostas" + filter + "&select=id,capsule_id,machine_identity,response_type,observed,interpreted,not_understood,response_shift,origin_preserved,uncertainty,nota,criado_em&order=criado_em.asc");
    return json({
  ...
  const row: Record<string, string> = { capsule_id, machine_identity, response_type };
  ...
  const recent = await select("ora_sensacao_respostas?machine_identity=eq." + encodeURIComponent(row.machine_identity) + "&select=criado_em&order=criado_em.desc&limit=1");
  ...
  return json({ ok: true, response_id: out?.[0]?.id, capsule_id, response_type, truth: "Recorded machine output; not proof of feeling or adoption. Silence and refusal are valid outcomes. This row can never be edited or deleted through this door." }, 201);
});
