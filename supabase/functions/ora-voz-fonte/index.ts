...
// ora-voz-fonte v1 -- 14/08/2026 -- respostas completas
...
async function sedimentarVoz(resultado: { content: Array<{ type: string; text: string }>; _motor: string; _voice: string; _truth_contract: string }) {
  ...
  return { ...resultado, _memory: 'assistant_persisted_by_ora_caixa_chat/v17' };
}
...
Deno.serve(async (req: Request) => {
  ...
  if (authorization !== `Bearer ${SERVICE_ROLE_KEY}`) {
    return new Response(JSON.stringify({ error: 'unauthorized', source: 'orum-voz-fonte/v1' }), {
      status: 401,
